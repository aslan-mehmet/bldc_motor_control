...
static void serial_flush_tx_buf(void)
{
	int16_t hold = -1;
...
void serial_encode_poll(uint8_t pid, uint8_t psize, void *pptr)
{
        serial_encode(pid, psize, pptr);
        serial_flush_tx_buf();
}
/*
...
static void serial_flush_rx_buf(void)
{
        int16_t hold = -1;
